...
router.post('/admin/product/insert', restrict, checkAccess, async (req, res) => {
    ...
    const doc = {
        ...
        productSchool: req.body.productSchool,
...
router.post('/admin/school/deleteimage', restrict, checkAccess, async (req, res) => {
    const db = req.app.db;

    // get the productImage from the db
    const product = await db.schools.findOne({ _id: common.getId(req.body.product_id) });
    if(!product){
        res.status(400).json({ message: 'Product not found' });
        return;
    }
    ...
    var found_item = false;
        if(product.productImage['id'] == req.body.productImage){
            found_item = true;
        }
    if(found_item){
        // set the productImage to null
            await db.schools.updateOne({ _id: common.getId(req.body.product_id) }, { $set: { productImage: null } });
        // remove the image from cloudinary
        console.log("product Id",req.body.productImage);
