...
$(document).ready(function (){
    ...
    $(document).on('click', '#btnSchoolAdd', function(e){
        $('#schoolNewForm').submit();
    });

    $('#schoolNewForm').on('submit', function(e){
        if(!e.isDefaultPrevented()){
            e.preventDefault();
            $.ajax({
                method: 'POST',
                ...
                data: {
                    schoolName: $('#usersName').val(),
                }
            })
            .done(function(msg){
                showNotification(msg.message, 'success', false, '/admin/school/edit/' + msg.userId);
            })
            .fail(function(msg){
                showNotification(msg.responseJSON.message, 'danger');
            });
    ...
    $(document).on('click', '#btnPincodeAdd', function(e){
        $('#pincodeNewForm').submit();
    });

    $(document).on('click', '#btnPincodeUpdate', function(e){
        $('#pincodeEditForm').submit();
    });


  

    $('#pincodeNewForm').on('submit', function(e){
        if(!e.isDefaultPrevented()){
            e.preventDefault();
            console.log("Pincode :::::::::");
            $.ajax({
                method: 'POST',
                url: '/admin/pincode/insert',
                data: {
                    pincode: $('#pincode').val(),
                    deliveryCharge: $('#delivery_charge').val()
                }
            })
            .done(function(msg){
                console.log("trueeeeeeeeeeeeeee");
                showNotification(msg.message, 'success', false, '/admin/pincode');
            })
            .fail(function(msg){
                showNotification(msg.responseJSON.message, 'danger');
            });
    ...
    $('#pincodeEditForm').on('submit', function(e){
        if(!e.isDefaultPrevented()){
            e.preventDefault();
            console.log("Pincode Update :::::::::");
            $.ajax({
                method: 'POST',
                url: '/admin/pincode/update',
                data: {
                    id:$('#_id').text(),
                    pincode: $('#pincode').val(),
                    deliveryCharge: $('#delivery_charge').val()
    ...
    $("#signin-tab-customerConfirm").on('submit', function(e){
        if(!e.isDefaultPrevented()){
            e.preventDefault();
        ...
        if(!e.isDefaultPrevented()){
            ...
            $.ajax({
                ...
                data: {
                    ...
                    productSchool: $('#productSchool').val(),
    ...
    $('.btn-delete-image-school').on('click', function(){
        if(confirm('Are you sure you want to delete this image?')){
            $.ajax({
                method: 'POST',
                url: '/admin/school/deleteimage',
                data: { product_id: $('#schoolId').val(), productImage: $(this).attr('data-id') }
            })
            .done(function(msg){
                showNotification(msg.message, 'success', true);
            })
    ...
    $(document).on('click', '#uploadSchoolButton', function(e){
        e.preventDefault();
        var formData = new FormData($('#uploadSchoolForm')[0]);
        formData.append('schoolId', $('#schoolId').val());

        // Upload file
        $.ajax({
            method: 'POST',
            url: '/admin/school/file/upload',
            processData: false,
            contentType: false,
            cache: false,
            data: formData
